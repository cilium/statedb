...
In addition to the radix tree based indexes StateDB also supports Longest Prefix
Match (LPM) indexes. These are useful for example for finding the object with the
closest IP prefix for a given IP address.
...
The `LPMIndex` allows defining an index for an arbitrary prefix:

...
type merger struct {
	obj       *object
	mergeFunc func(old any) any
}

func (m merger) merge(old *object) *object {
	if old != nil && m.mergeFunc != nil {
		m.obj.data = m.mergeFunc(old.data)
	}
	return m.obj
}
...
func (txn *writeTxnState) modify(meta TableMeta, guardRevision Revision, newData any, merge func(any) any) (*object, bool, <-chan struct{}, error) {
	...
	binary.BigEndian.PutUint64(obj.revisionKey[:], revision)
	idIndexTxn := txn.mustIndexWriteTxn(meta, PrimaryIndexPos)
	idKey := idIndexTxn.objectToKey(obj)

	oldObj, oldExists, watch := idIndexTxn.modify(
		idKey,
		obj,
		merger{obj, merge}.merge)
	...
	if oldExists {
		txn.mustIndexWriteTxn(meta, RevisionIndexPos).delete(oldObj.revisionKey[:])
	}
	revIndexTxn.insert(obj.revisionKey[:], obj)

	// If it's new, possibly remove an older deleted object with the same
	// primary key from the graveyard.
	if !oldExists {
		old, existed := txn.mustIndexWriteTxn(meta, GraveyardIndexPos).delete(idKey)
		if existed {
			txn.mustIndexWriteTxn(meta, GraveyardRevisionIndexPos).delete(old.revisionKey[:])
		}
	}
	...
	for _, indexer := range meta.secondary() {
		...
		if oldExists {
			...
			indexTxn.reindex(idKey, nil, obj)
...
func (txn *writeTxnState) delete(meta TableMeta, guardRevision Revision, data any) (*object, bool, error) {
	...
	txn.mustIndexWriteTxn(meta, RevisionIndexPos).delete(obj.revisionKey[:])
	...
	if txn.hasDeleteTrackers(meta) {
		...
		binary.BigEndian.PutUint64(deadObj.revisionKey[:], revision)
		if _, existed, _ := graveyardIndex.insert(idKey, deadObj); existed {
			panic("BUG: Double deletion! Deleted object already existed in graveyard")
		}
		txn.mustIndexWriteTxn(meta, GraveyardRevisionIndexPos).insert(
			deadObj.revisionKey[:], deadObj)
	}

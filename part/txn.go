...
type Txn[T any] struct {
	...
	// watches contains the lazy watch channels of cloned nodes that should be
	// closed when the transaction is committed.
	watches []*lazyWatchChannel
...
func (txn *Txn[T]) InsertWatch(key []byte, value T) (old T, hadOld bool, watch <-chan struct{}) {
	...
	old, _, hadOld, target, txn.root = txn.insert(txn.root, key, value)
	validateTree(txn.root, nil, txn.watches, txn.txnID)
	if !hadOld {
		txn.size++
	}
	if txn.opts.rootOnlyWatch() {
		target = watchTarget{direct: txn.rootWatch}
	}
	watch = target.channel()
	return
}
...
func (txn *Txn[T]) ModifyWatch(key []byte, value T, mod func(T, T) T) (old T, newValue T, hadOld bool, watch <-chan struct{}) {
	var target watchTarget
	old, newValue, hadOld, target, txn.root = txn.modify(txn.root, key, value, mod)
	validateTree(txn.root, nil, txn.watches, txn.txnID)
	if !hadOld {
		txn.size++
	}
...
func (txn *Txn[T]) addWatch(watch *lazyWatchChannel) {
...
func (txn *Txn[T]) insert(root *header[T], key []byte, value T) (oldValue T, newValue T, hadOld bool, watch watchTarget, newRoot *header[T]) {
	return txn.modify(root, key, value, nil)
}

func (txn *Txn[T]) modify(root *header[T], key []byte, newValue T, mod func(T, T) T) (oldValue T, newValueOut T, hadOld bool, watch watchTarget, newRoot *header[T]) {
	txn.dirty = true
	fullKey := key
	...
	if root == nil {
		...
		return oldValue, newValueOut, false, watchTarget{lazy: &leaf.watch}, leaf.self()
	}
	...
	if this.isLeaf() {
		...
		this = this.cloneWithSharedWatch()
	} else {
		this = txn.cloneNode(this)
	}
...
func (txn *Txn[T]) delete(root *header[T], key []byte) (oldValue T, hadOld bool, newRoot *header[T]) {
	...
	if target == root {
		...
		case root.size() == 1:
			...
			childClone := child.cloneWithSharedWatch()
...
func (txn *Txn[T]) removeChild(parent *header[T], index int) (newParent *header[T]) {
	...
	case parent.kind() == nodeKind256 && size <= 129:
		demotedNode := &node128[T]{}
		parent.cloneHeaderTo(&demotedNode.header, parent.watch.enabled())
		demoted := demotedNode.self()
...
// if it is not.
func validateTree[T any](node *header[T], parents []*header[T], watches []*lazyWatchChannel, maxTxnID uint64) {
	if !runValidation {
		return
	}
...
func validateRemovedWatches[T any](oldRoot *header[T], newRoot *header[T]) {
	...
	// An unmaterialized watch is uniquely identified by its slot. Watches only
	// share state after an identity has been installed, in which case the
	// identity pointer is their common key. Do not use loadOrCreate here: tree
	// validation must not change whether a watch has been observed.
	type watchIdentity struct {
		materialized   *atomicWatchPointer
		unmaterialized *lazyWatchChannel
	}
	identitySnapshots := map[*lazyWatchChannel]*atomicWatchPointer{}
	stable := true
	watchIdentityFor := func(w *lazyWatchChannel) watchIdentity {
		identity := w.identity.Load()
		if previous, found := identitySnapshots[w]; found {
			stable = stable && previous == identity
		} else {
			identitySnapshots[w] = identity
		}
		if identity != nil {
			return watchIdentity{materialized: identity}
		}
		return watchIdentity{unmaterialized: w}
	}
	formatWatchIdentity := func(identity watchIdentity) string {
		if identity.materialized != nil {
			return fmt.Sprintf("%p", identity.materialized)
		}
		return fmt.Sprintf("lazy:%p", identity.unmaterialized)
	}
	...
	summarizeNodeStructure = func(node *header[T]) func() string {
		...
		return func() string {
			...
			s := fmt.Sprintf("K:%d S:%d W:%s L:[%s] C:[%s]", node.kind(), node.size(), formatWatchIdentity(watchIdentityFor(&node.watch)), leafS, childS)
			nodeStruct[node] = s
			return s
		}
	}

	var collectWatches func(depth int, watches map[watchIdentity]func() string, node *header[T])
	collectWatches = func(depth int, watches map[watchIdentity]func() string, node *header[T]) {
		if node == nil {
			return
		}
		if !node.watch.enabled() {
			panic("nil watch channel")
		}
		watches[watchIdentityFor(&node.watch)] = summarizeNodeStructure(node)
		if leaf := node.getLeaf(); leaf != nil && !node.isLeaf() {
			watches[watchIdentityFor(&leaf.watch)] = summarizeNodeStructure(leaf.self())
		}
		for _, child := range node.children() {
			if child != nil {
	...
	oldWatches := map[watchIdentity]func() string{}
	collectWatches(0, oldWatches, oldRoot)
	newWatches := map[watchIdentity]func() string{}
	collectWatches(0, newWatches, newRoot)
	if !stable {
		// An observer materialized an unchanged watch between scans. Retry
		// with its now-stable identity rather than reporting it as removed.
		validateRemovedWatches(oldRoot, newRoot)
		return
	}
	...
	for watch, desc := range oldWatches {
		if watch.materialized == nil || !watch.materialized.isClosed() {
			oldRoot.printTree(0)
			fmt.Println("---")
			newRoot.printTree(0)
			panic(fmt.Sprintf("dropped watch channel %s not closed %s", formatWatchIdentity(watch), desc()))
		}
	}
}

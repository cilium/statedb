...
func prefixSearch[T any](root *node[T], key []byte) (*Iterator[T], <-chan struct{}) {
	this := root
...
type Iterator[T any] struct {
	next [][]*node[T] // the next set of edges to explore
}

func newIterator[T any](start *node[T]) *Iterator[T] {
	if start == nil {
		return &Iterator[T]{nil}
	}
	return &Iterator[T]{[][]*node[T]{{start}}}
}

func lowerbound[T any](start *node[T], key []byte) *Iterator[T] {
	...
	for {
		...
		case 0:
			if len(this.prefix) == len(key) {
				// Exact match.
				edges = append(edges, []*node[T]{this})
				break loop
			} else if len(key) == 0 {
...
func (it *Iterator[T]) Next() (key []byte, value T, ok bool) {
	for len(it.next) > 0 {
		...
		if node.size() > 0 {
			if node.kind() == nodeKind256 {
				it.next = append(it.next, node.node256().children[:])
			} else {
